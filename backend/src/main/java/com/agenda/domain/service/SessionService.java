...
public class SessionService {
  ...
  public SessionEntity create(
      String title, LocalDateTime plannedAt, Integer duration, SessionMethod method, Long userId) {
  ...
  public List<SessionEntity> getAll(Long userId) {
    ...
    return sessionRepository.findByUser(user).stream().map(sessionConverter::toEntity).toList();
  }

  public SessionEntity getById(Long sessionId) {
    SessionModel model = sessionRepository.findById(sessionId);
    if (model == null) throw new RuntimeException("Session not found");

    return sessionConverter.toEntity(model);
  }

  @Transactional
  public SessionEntity update(
      Long sessionId,
      String title,
      Integer duration,
    ...
    if (duration != null) model.setDuration(duration);

    if (plannedAt != null) model.setPlannedAt(plannedAt);

    if (method != null) model.setMethod(method);
  ...
  public SessionEntity start(Long sessionId) {
    ...
    if (model.getStatus() == SessionStatus.DONE) throw new RuntimeException("Session already done");
